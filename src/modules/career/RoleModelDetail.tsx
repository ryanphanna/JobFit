...
export const RoleModelDetail: React.FC<RoleModelDetailProps> = ({
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-800">
            ...
            <div className="space-y-4">
                <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-100 dark:border-slate-700">
                    <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">Added On</div>
                    <div className="text-sm font-bold text-slate-700 dark:text-slate-300">{new Date(roleModel.dateAdded).toLocaleDateString()}</div>
                </div>
                <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-100 dark:border-slate-700">
                    <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">Verification Status</div>
    ...
    return (
        <div className="bg-white dark:bg-slate-950 h-full flex flex-col animate-in fade-in duration-500">
            <DetailHeader
            ...
            >
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-xl p-8 shadow-sm border border-slate-200 dark:border-slate-800">
                            <div className="flex items-center gap-3 mb-6">
                                ...
                                <div>
                                    <h4 className="font-bold text-slate-900 dark:text-white">Distilled Hard Skills</h4>
                                    <p className="text-xs text-slate-500 dark:text-slate-400">Key technical competencies from this path</p>
                                </div>
                            </div>
                            <div className="flex flex-wrap gap-2">
                                {roleModel.topSkills.map((skill, idx) => (
                                    <span
                                        key={idx}
                                        className="text-sm font-medium text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700"
                                    >
                                        {skill}
                                    </span>
                                ))}
                        ...
                        <div className="p-8">
                            {(!roleModel.experience || roleModel.experience.length === 0) ? (
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">No experience blocks extracted yet.</p>
                                    <p className="text-slate-400 text-[10px] mt-2">Try re-analyzing the profile for deeper insights.</p>
                                </div>
                            ) : (
                                <div className="space-y-12 relative before:absolute before:left-4 before:top-2 before:bottom-2 before:w-px before:bg-slate-100 dark:before:bg-slate-800">
                                    {roleModel.experience.map((block, i) => (
                                        <div key={i} className="relative pl-12 group">
                                            {/* Dot */}
                                            <div className="absolute left-0 top-1.5 w-8 h-8 rounded-full bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 flex items-center justify-center z-10 group-hover:border-emerald-500 transition-colors">
                                                <div className="w-2 h-2 rounded-full bg-slate-300 dark:bg-slate-700 group-hover:bg-emerald-500 transition-colors" />
                                            </div>
                                            ...
                                            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                                                <div className="flex-1">
                                                    ...
                                                    <h4 className="text-lg font-black text-slate-900 dark:text-white leading-tight mb-1">
                                                        {block.title}
                                                    </h4>
                                                    <div className="text-sm font-bold text-slate-500 dark:text-slate-400 mb-4 flex items-center gap-2">
                                                        {block.organization}
                                                        <span className="text-slate-300">•</span>
                                                        <span className="text-slate-400 font-medium">{block.dateRange}</span>
                                                    </div>
                                                    ...
                                                    <ul className="space-y-3">
                                                        {block.bullets.map((bullet, idx) => (
                                                            <li key={idx} className="flex gap-3 text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                                                                <div className="w-1 h-1 rounded-full bg-slate-300 dark:bg-slate-700 mt-2 shrink-0" />
                                                                {bullet}
                                                            </li>
                                                        ))}
                                                    </ul>
                ...
                {activeTab === 'context' && (
                    <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                        <div className="p-8">
                            <h4 className="flex items-center gap-2 font-bold text-slate-900 dark:text-white mb-6 uppercase text-sm tracking-wider">
