...
export default function History({ jobs, onSelectJob, onDeleteJob }: HistoryProps) {
    ...
    const getStatusBadge = (status: SavedJob['status']) => {
        switch (status) {
            ...
            case 'rejected': return <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700"><XCircle className="w-3 h-3" /> Rejected</span>;
            default: return null;
        }
    }
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            <div className="flex items-center justify-between mb-2">
                <div>
                    <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Application History</h2>
                    <p className="text-slate-500 dark:text-slate-400 text-sm">Track your analyzed jobs and applications</p>
                </div>
                <span className="text-xs font-medium text-slate-500 bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full">{filteredJobs.length} Jobs</span>
            </div>
            ...
            <div className="relative mb-8">
                ...
                <input
                    ...
                    className="w-full pl-12 pr-4 py-3.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-base focus:outline-none focus:ring-4 focus:ring-indigo-100 dark:focus:ring-indigo-900/30 focus:border-indigo-500 transition-all placeholder:text-slate-400 shadow-sm transition-all"
                />
                {searchQuery && (
                    <button
                        onClick={() => setSearchQuery('')}
                        className="absolute right-4 top-1/2 -translate-y-1/2 p-2 text-slate-300 hover:text-slate-500 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                    >
                        <X className="w-4 h-4" />
                    </button>
                )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {jobs.length === 0 ? (
                    <div className="col-span-full py-20 text-slate-500 flex flex-col items-center justify-center text-center">
                        ...
                        <p className="text-lg font-medium text-slate-900 dark:text-white">No jobs analyzed yet</p>
                        <p className="text-sm text-slate-500 dark:text-slate-400">Paste a job URL on the home page to get started.</p>
                    </div>
                    ...
                    <div className="col-span-full py-20 bg-slate-50/50 dark:bg-slate-900/50 rounded-[2.5rem] border border-dashed border-slate-200 dark:border-slate-800 flex flex-col items-center justify-center text-center">
                        <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl shadow-sm mb-4">
                            <Search className="w-8 h-8 text-slate-300 dark:text-slate-700" />
                        </div>
                        <h3 className="text-slate-900 dark:text-white font-medium mb-1">No jobs found</h3>
                        <p className="text-slate-500 dark:text-slate-400 text-sm mb-4">No applications match "{searchQuery}"</p>
                        <button
                            onClick={() => setSearchQuery('')}
